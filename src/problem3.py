...
def problem3(point, length, delta, window):
    ...
    start = rg.Point(point.x, point.y + delta)
    end = rg.Point(point.x + length + 20, point.y + delta)

    for k in range((length // delta)):
            line_h = rg.Line(start, end)
            line_h.attach_to(window)
            line_h.thickness = 3
            start = rg.Point(point.x, start.y + delta)
            end = rg.Point(end.x + 20, end.y + delta)
            if (k + 1) == 1:
                line_h.color = 'cyan'
            elif (k + 1) == 2:
